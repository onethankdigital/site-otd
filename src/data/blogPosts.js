...
Aqui está a diferença entre um robô e um SDR digital. O agente de IA precisa ser construído sobre uma <strong>metodologia de vendas real</strong> — frameworks consultivos como o SPIN Selling — para conduzir a conversa por etapas: entender a situação do lead, identificar o problema, dimensionar a implicação e conectar com a necessidade de solução.<br><br>Um agente bem construído:<br><br>• responde em segundos, em linguagem natural, no tom da marca;<br>• faz perguntas de qualificação sem parecer um interrogatório;<br>• identifica se o lead tem perfil (segmento, porte, urgência);<br>• respeita a LGPD no tratamento dos dados coletados;<br>• e sabe a hora de <strong>parar e transferir para o humano</strong> — essa é uma regra de ouro.<br><br>
...
De nada adianta qualificar bem se a informação morre no chat. Cada conversa precisa gerar automaticamente:<br><br>• <strong>card no CRM</strong> com os dados de qualificação preenchidos;<br>• <strong>classificação de temperatura</strong> (pronto para reunião, nutrir, sem perfil);<br>• <strong>agendamento direto na agenda</strong> do responsável comercial, quando o lead está quente;<br>• <strong>notificação para o time</strong> com o resumo do caso.<br><br>E, para fechar o ciclo de inteligência, o ponto de entrada — o botão de WhatsApp do site — deve ser rastreado no GA4, permitindo saber exatamente quais páginas e campanhas geram os leads que viram reunião. O vendedor começa o dia com a fila organizada, não com uma pilha de mensagens soltas para destrinchar.`
    ...
    faq: [
      ...
      },
      {
        ...
        resposta: "Depende inteiramente da base técnica. Operações construídas sobre a API oficial da Meta (Cloud API), com fluxos receptivos e respeito às políticas da plataforma, têm risco mínimo. Já ferramentas não-oficiais — que simulam o WhatsApp Web para automatizar mensagens — violam os termos da Meta e podem levar ao banimento permanente do número, mesmo em fluxos iniciados pelo cliente. A escolha da infraestrutura é a decisão mais importante do projeto."
      },
      {
